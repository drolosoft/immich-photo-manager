...
Tags are the visible state a user acts on inside Immich. Notes are the why, and they are invisible in the Immich UI and not searchable there. The key holds two lists, `reviews` (a verdict plus its reason) and `actions` (what the plugin did plus the detail worth keeping), each capped at its newest 10 entries, because this is a memory and not a log.

Verdicts come from a closed vocabulary, `keep`, `delete_candidate`, `duplicate_of` and `needs_check`, so they stay comparable across sessions and models. Nothing writes a note on its own: a tool call does, after a decision. And `clear_asset_notes` removes only this plugin's key, never another app's.
...
It also opened its report with a warning rather than a claim:
...
Ratings run 1 to 5, with -1 for rejected, and `rating=0` is refused up front because Immich 3.x rejects it. One call never runs on the first try: `merge_people` is irreversible, so it previews instead:
